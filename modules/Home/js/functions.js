...
function dateBetween(dateBetween, searchTerm = null, page = null, filter = 0, status = null, type = null, dropped = null, changeColumns = 1) {
    ...
    $.ajax({
        ...
        success: function (check) {
            ...
            // console.log("data =teak djh fk j ",data.team_count);
            $('#myTeamCount').html(teamCount);
...
function filterHelper(ref) {
    ...
    if (ref == 'opportunity-filter') {
        var day = Cookies.get('day');
        dateBetween(day, '', '', 1, filterStatus, filterType, '', 0);
    } else {
...
function fetchDeselectDialog(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            document.getElementById('tag_opporunity_id').value = parsed_data.opportunity_id;
            dialog.style.display = "block";
            $('#hidden_user').val(parsed_data.msuname);
            var temp = parsed_data.msuid.split(',');
            $('#deselect_members_tag').html(parsed_data.opporunity_tag);
            initSelect2();
        }
    })
...
function fontSymbolOut() {
    $('#deselect-drop-icon').removeClass('fa-times').addClass('fa-caret-down');
}
function fontSymbolIn() {
    $('#deselect-drop-icon').removeClass('fa-caret-down').addClass('fa-times');
}
...
function removeSeleted() {
    $("#deselect_members option[selected]").each(function () {
