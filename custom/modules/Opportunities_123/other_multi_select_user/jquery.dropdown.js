    ...
    $.each(data, function (index, val) {
      ...
      if (isSelected) {
        ...
        // console.log(val.name.length);
         console.log('1st');
        var res = val.name.split("/");
        // console.log(res[0]+' '+res[1]);
        console.log(res[0]);
        name.push('<span class="dropdown-selected">' + res[0] + '<i class="del" data-id="' + val.id + '"></i></span>');
        selectAmount++;
      }
      // 判断是否有分组
      if (hasGroup) {
      ...
      $.each(_config.data, function (key, item) {
        ...
        if (item.selected) {
         ...
         // console.log('2nd');
            ...
            _dropdown.name.push('<span class="dropdown-selected">'+res[0]+'<i class="del" data-id="' + item.id + '"></i></span>');
        }
      });
