    ...
    //	echo $log_in_user_id;
        ...
        while($row3 = $GLOBALS['db']->fetchByAssoc($result3)) 
        {
            ...
            $check_team_lead = $row3['teamheirarchy_c'];
			 ...
			 if(  $check_mc =="yes"|| $log_in_user_id == $created_by || $log_in_user_id == "1" || in_array($log_in_user_id, $team_func_array1)||in_array($log_in_user_id, $others_id_array)  || in_array($log_in_user_id, $team_func_array) || $log_in_user_id == $approver1 || $log_in_user_id == $assigned) {
...
//---------------------Approval Buttons-----------------
    ...
    $next_status='';
     try{ 
        ...
        //------------For approver-----------------------------------------------------------------
			if(in_array($log_in_user_id, $team_func_array1)||$log_in_user_id==$new_approver){
			    
			  	$sql2="SELECT * FROM approval_table  WHERE approver_rejector='".$log_in_user_id."' AND opp_id='".$opportunity_id."' AND status='".$status."' AND rfp_eoi_published='".$rfp_eoi_published."'";
			    ...
			    if($result2->num_rows>0){
			      ...
			      if( $Approved == 0 && $Rejected == 0 && $pending==1)	{
			          ...
			          echo json_encode(array("status"=> true, "button"=>'approve_reject'));
			      }
		...
		//---------------For sender----------------------------------------------------------------		
                    	    	...
                    	    	$sql2="SELECT * FROM approval_table  WHERE sender='".$log_in_user_id."' AND opp_id='".$opportunity_id."' AND status='".$status."' AND rfp_eoi_published='".$rfp_eoi_published."'";
                    			    ...
                    			    if($value==0 && $value1==0 && $value2==1)	{
                    			         $button = "pending";
                                     echo json_encode(array("status"=> true, "button"=>$button));
                                
                    			        
                    			    }
                    			    else if($value==0 && $value1==1 && $value2==0)	{
                    			          $button = "send_approval_same";
                                     echo json_encode(array("status"=> true, "button"=>$button,'message'=>'rejected'));
                    			    }
                    			     else if($value==1 && $value1==0 && $value2==0)	{
        ...
        //-----------------For deligate------------------------------------------------------------      
			  	...
			  	$sql2="SELECT * FROM approval_table  WHERE delegate_id='".$log_in_user_id."' AND opp_id='".$opportunity_id."' AND status='".$status."' AND rfp_eoi_published='".$rfp_eoi_published."'";
			...
			while($ro = $GLOBALS['db']->fetchByAssoc($resul) )
				{
				 ...
				 $team_func_array2 = explode(',',  $new_approver);
			  	...
			  	$sql2="SELECT * FROM approval_table  WHERE approver_rejector='".$log_in_user_id."' AND opp_id='".$opportunity_id."'  AND rfp_eoi_published='".$rfp_eoi_published."'  ORDER BY id DESC LIMIT 1";
			    ...
			    if($result2->num_rows>0){
			  	...
			  	$sql2="SELECT * FROM approval_table  WHERE delegate_id='".$log_in_user_id."' AND opp_id='".$opportunity_id."'  AND rfp_eoi_published='".$rfp_eoi_published."'  ORDER BY id DESC LIMIT 1";
...
// 				}
		...
		//	echo	json_encode($approver_array);
		//	die();
                	$sql4="SELECT * FROM approval_table  WHERE  opp_id='".$opportunity_id."' AND status='".$status."' AND rfp_eoi_published='".$rfp_eoi_published."'";
                				          	...
                				          	$sql="UPDATE approval_table SET Approved=0,Rejected=0,pending=1 WHERE approver_rejector='".$approvers."' AND opp_id='".$opportunity_id."' AND status='".$status."' AND rfp_eoi_published='".$rfp_eoi_published."' " ;		     
                                    			...
                                    			if($db->query($sql)==TRUE){
                                    			 ...
                                    			 //  $alert = BeanFactory::newBean('Alerts');
                                    				// 		$alert->name =$opp_name;
                                    				// 		$alert->description = 'opportunity is sent for approval in '. $apply.' status Please Review';
                                    				...
                                    				// //emails
                                    				// $template = 'The '.$opp_name.' opportunity is sent for approval  in '. $apply.' status Please Review : '.$base_url.'?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DOpportunities%26offset%3D7%26stamp%3D1595474141045235900%26return_module%3DOpportunities%26action%3DDetailView%26record%3D'.$opportunity_id;
                                    				...
                                    				// 	    $mail->Subject = ''. $apply.' Opportunity is sent for  approval'; 
                                    				// 		$mail->Body =$template;
				...
				if ($message=="true"){
				   echo "Opportunity ".$name." has been sent to ".$approver_name." for approval";
				}
...
//                  	$sql3 ='SELECT * FROM users WHERE id="'.$approver.'"';
// 			$result3 = $GLOBALS['db']->query($sql3);
			
// 			while($row3 = $GLOBALS['db']->fetchByAssoc($result3) )
// 				{
// 				    $email=$row3['user_name'];
     ...
     try{ 
    	...
    	$db = \DBManagerFactory::getInstance();
         ...
         if($result4->num_rows>0){
              ...
              $sql='UPDATE approval_table SET Approved="'.$Approved.'" ,Rejected="'.$Rejected.'",pending="'.$pending.'",delegate_comments="'.$Comments.'",delegate_date_time="'.$date_time.'" WHERE opp_id="'.$opportunity_id.'"  AND delegate_id="'.$log_in_user_id.'" AND status="'.$status.'" AND rfp_eoi_published="'.$rfp_eoi_published.'"';
              $result= $GLOBALS['db']->query($sql);
              
             
           
         }
                           	$sql2="SELECT * FROM approval_table  WHERE  opp_id='".$opportunity_id."' AND status='".$status."' AND rfp_eoi_published='".$rfp_eoi_published."'";
         ...
         if($result4->num_rows>0){
              ...
              $sql='UPDATE approval_table SET Approved="'.$Approved.'" ,Rejected="'.$Rejected.'",pending="'.$pending.'",comments="'.$Comments.'",date_time="'.$date_time.'" WHERE opp_id="'.$opportunity_id.'"  AND approver_rejector="'.$log_in_user_id.'" AND status="'.$status.'" AND rfp_eoi_published="'.$rfp_eoi_published.'"';
              $result= $GLOBALS['db']->query($sql);
...
// 				$sql3 ='SELECT * FROM users WHERE id="'.$created_by.'"';
// 			$result3 = $GLOBALS['db']->query($sql3);
...
// 			     $alert = BeanFactory::newBean('Alerts');
// 						$alert->name =$opp_name;
// 						$alert->description = 'opportunity is approved for status '. $apply.' ';
// 						$alert->url_redirect = $base_url.'?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DOpportunities%26offset%3D7%26stamp%3D1595474141045235900%26return_module%3DOpportunities%26action%3DDetailView%26record%3D'.$opportunity_id;
// 						$alert->target_module = 'Opportunities';
...
//               	//emails
// 				$template = 'The '.$opp_name.' opportunity is  approved for status '. $apply.' . Please Review : '.$base_url.'?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DOpportunities%26offset%3D7%26stamp%3D1595474141045235900%26return_module%3DOpportunities%26action%3DDetailView%26record%3D'.$opportunity_id;
// 						require_once('include/SugarPHPMailer.php');
// 						include_once('include/utils/db_utils.php');
// 					    $emailObj = new Email();  
// 					    $defaults = $emailObj->getSystemDefaultEmail();  
...
// 				$update_query="UPDATE opportunities_cstm SET status_c='".$next_status."' WHERE id_c='".$opportunity_id."'";
...
// 			    $alert = BeanFactory::newBean('Alerts');
// 						$alert->name =$opp_name;
// 						$alert->description = 'opportunity is rejected for status '. $apply.' ';
// 						$alert->url_redirect = $base_url.'?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DOpportunities%26offset%3D7%26stamp%3D1595474141045235900%26return_module%3DOpportunities%26action%3DDetailView%26record%3D'.$opportunity_id;
// 						$alert->target_module = 'Opportunities';
// 						$alert->assigned_user_id = $created_by;
// 						$alert->type = 'info';
...
// 					//emails
// 				$template = 'The '.$opp_name.' opportunity is rejected for status '. $apply.' . Please Review : '.$base_url.'?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DOpportunities%26offset%3D7%26stamp%3D1595474141045235900%26return_module%3DOpportunities%26action%3DDetailView%26record%3D'.$opportunity_id;
// 						require_once('include/SugarPHPMailer.php');
// 						include_once('include/utils/db_utils.php');
// 					    $emailObj = new Email();  
// 					    $defaults = $emailObj->getSystemDefaultEmail();  
...
public function action_fetch_reporting_manager()
    {
     ...
     echo json_encode(array("status"=>true, "reporting_id"=>$reporting_id,"reportee"=>$email_id,"fullname"=>$full_name, "user_id" => $usersid,"email" => $email, "name" => $fullname,"teamfunction"=>$fun_array,"other_user_id" => $others_id_array));
...
public function action_untagged_users_list(){
     try{  
       ...
       $op_id = $_POST['opps_id'];
		...
		$sql1 = "SELECT  id, user_name,CONCAT(IFNULL(first_name,''), ' ', IFNULL(last_name,'')) AS fullname FROM users WHERE id NOT IN ('".implode("','",$reporting)."')";
		 	$result1 = $GLOBALS['db']->query($sql1);
			while($row1 = $GLOBALS['db']->fetchByAssoc($result1))
			{
	    		    array_push($users_id,$row1['id']);
