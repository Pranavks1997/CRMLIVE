...
class DocumentsController extends SugarController
{
            ...
            if ($check_mc == 'yes')
            {
                ...
                if ($log_in_user_id == $assigned_id)
                {
                    ...
                    echo json_encode(array(
                        'reporting_name' => $mc_name,
                        'reporting_id' => $log_in_user_id,
                        'mc' => "yes",
                        'login_user' => $log_in_user_id
                    ));
    ...
    public function action_send_approval()
    {
        try
        {
            ...
            $body = 'Document "' . $document_name . '" is received for approval from "' . $current_user->first_name . ' ' . $current_user->last_name . '" <br><br> Click here to view: www.ampersandcrm.com';
            $to = $approver_email;
            ...
            echo json_encode(["button" => "hide", "message" => "Approval request has been sent to " . $approver]);
    ...
    public function action_approve()
    {
        try
        {
            ...
            if ((bool)$assigned_to_name)
            {
                ...
                echo json_encode(['button' => 'hide', 'message' => 'Document "' . $document_name . '" approved successfully.']);
            }
    ...
    public function action_reject()
    {
        try
        {
            ...
            if ((bool)$assigned_to_name)
            {
                ...
                echo json_encode(['button' => 'hide', 'message' => 'Document "' . $document_name . '" rejected successfully.']);
            }
        }
        catch(Exception $e)
        {
    ...
    public function action_approval_buttons()
    {
        try
        {
            ...
            else
            {
                ...
                {
                    echo json_encode(array(
                        "message" => "show_completed"
                    ));
                }
                else if ($log_in_user_id == $assigned_id)
                {
                    echo json_encode(array(
                        "message" => "show_send_approval"
                    ));
                }
                else
                {
