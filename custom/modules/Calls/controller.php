...
public function action_reminderActivity(){
  ...
  //  $start = date('G:i:s',$startTime);
   ...
   while ($row = $GLOBALS['db']->fetchByAssoc($result)){ 
      ...
      if  (strtolower($row['frequency']) == 'daily'){
           ...
           $this->activityQuery($row['activity_id'],$db );
      ...
      // montly
      elseif (strtolower($row['frequency']) == 'montly') {
          # code...
          $created_date = $row['updated_at'];
          $activity_date_time = strtotime($created_date);
...
public function activityQuery($activity_id,$db){
    //  print_r($activity_id);
    //       die();
  ...
  $this->send_email($row['name'], $data['user_name']);
}

public function send_email($activity_name,$email){
